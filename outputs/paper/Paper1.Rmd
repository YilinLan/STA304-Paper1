...
```{r hist, fig.cap="Count for Different Original On Fire Area", fig.width=5, fig.height=4.5,message=FALSE, echo=FALSE}
...
```{r point, fig.cap="Estimated Dollar Loss by Civilian Casualties and Building Status", fig.width=4, fig.height=2,message=FALSE, echo=FALSE}
...
```{r point2, fig.cap="Estimated Dollar Loss by Number of Exposure Fires and Possible Cause", fig.width=4, fig.height=2.7,message=FALSE, echo=FALSE}
...
```
Last, in figure 4, I illustrate the medium of estimated property loss under different building status. It can be seen that if the fire happened in an under-construction building, the medium property loss will be high at 500000 dollars.^[https://github.com/YilinLan/STA304-Paper1]
...
summary_table<-data1%>%group_by(Building_Status)%>%summarise(min=min(Estimated_Dollar_Loss), median=median(Estimated_Dollar_Loss),mean=mean(Estimated_Dollar_Loss),max=max(Estimated_Dollar_Loss),Q1=quantile(Estimated_Dollar_Loss,0.25),Q3=quantile(Estimated_Dollar_Loss,0.25),IQR=Q3-Q1,sd=sd(Estimated_Dollar_Loss),Small_Outliers=sum(Estimated_Dollar_Loss<Q1-1.5*IQR),Large_Outliers=sum(Estimated_Dollar_Loss<Q3+1.5*IQR))
